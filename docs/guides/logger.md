...
config = LoggerConfig(
    ...
    json_logging=False,            # JSON формат (рекомендуется для продакшена)
    log_dir=Path("logs"),       # Директория для файлов логов
    log_file="metaexpert.log",  # Имя основного файла логов
    log_trade_file="trades.log", # Имя файла для трейдинг логов
    log_error_file="errors.log", # Имя файла для ошибок

...
- **Структурированность:** Все логи должны быть структурированными для простого анализа и парсинга
...
- **Процессоры `structlog`:** В `logger/processors.py` будут реализованы кастомные обработчики для добавления специфичного для домена контекста (например, `add_expert_context`, `add_trade_details`).
- **Обработчики (Handlers):** Помимо стандартных `FileHandler` и `StreamHandler`, в `logger/handlers.py` могут быть добавлены обработчики для отправки уведомлений о критических ошибках.
...
- **`TRADE`:** Специализированный уровень для логирования исключительно торговых событий (создание, исполнение ордеров). Сообщения этого уровня направляются только в `trades.log`.
...
При стандартном логировании используется формат:
...
При включенной опции `structured_logging` логи записываются в формате JSON:
...
- **`trades.log`:** Специализированный лог только для торговых операций (создание/исполнение/отмена ордеров, открытие/закрытие позиций). Записывается в формате, легко поддающемся парсингу (например, CSV или JSON-строки).

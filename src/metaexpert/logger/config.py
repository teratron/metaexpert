...
class LogConfiguration(BaseModel):
    ...
    log_file: str | None = Field(default=None, description="Backward compatibility for log_file parameter")
    trade_log_file: str | None = Field(default=None, description="Backward compatibility for trade_log_file parameter")
    error_log_file: str | None = Field(default=None, description="Backward compatibility for error_log_file parameter")
    log_max_file_size: int | None = Field(default=None, description="Backward compatibility for log_max_file_size parameter")
    log_backup_count: int | None = Field(default=None, description="Backward compatibility for log_backup_count parameter")
    async_logging: bool | None = Field(default=None, description="Backward compatibility for async_logging parameter")
    structured_logging: bool | None = Field(default=None, description="Backward compatibility for structured_logging parameter")
            ...
            if env_value is not None and field not in data:
                ...
                if field in ['enable_async', 'enable_structured_logging', 'enable_contextual_logging', 'mask_sensitive_data', 'disk_space_check']:
                    ...
                    try:
                        ...
                        raise ValueError(f"Environment variable {env_var} must be an integer") from None
                else:

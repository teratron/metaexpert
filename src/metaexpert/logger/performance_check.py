...
def check_performance_latency(config: LogConfiguration, num_logs: int = 1000) -> dict:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        return {
            ...
            'p95_time_ms': sorted(log_times)[int(0.95 * len(log_times))] if log_times else 0,
            'p99_time_ms': sorted(log_times)[int(0.99 * len(log_times))] if log_times else 0,
        }


def validate_performance_requirements(metrics: dict, 
                                   avg_threshold_ms: float = 10.0,
                                   p95_threshold_ms: float = 10.0) -> tuple[bool, List[str]]:
    ...
    if metrics['avg_time_per_log_ms'] > avg_threshold_ms:
        issues.append(f"Average log time ({metrics['avg_time_per_log_ms']:.2f}ms) "
    ...
    if metrics['p95_time_ms'] > p95_threshold_ms:
        issues.append(f"95th percentile log time ({metrics['p95_time_ms']:.2f}ms) "
    ...
    if metrics['percent_under_10ms'] < 95:
        issues.append(f"Only {metrics['percent_under_10ms']:.2f}% of operations "
    ...
    if metrics['max_time_ms'] > 50:
        issues.append(f"Maximum log time ({metrics['max_time_ms']:.2f}ms) "
...
def run_performance_check():
    ...
    print(f"Asynchronous logging metrics:")
    print(f"  Total time: {async_metrics['total_time_ms']:.2f}ms for {async_metrics['num_logs']} logs")
    ...
    print(f"Synchronous logging metrics:")

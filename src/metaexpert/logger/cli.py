...
def add_logging_options(app: typer.Typer):
    ...
    def logging_options(
        ...
        log_directory: Optional[str] = typer.Option(
            None, "--log-directory",
            help="Directory path for log files"
        ),
        expert_log_file: Optional[str] = typer.Option(
            None, "--expert-log-file",
            help="Filename for expert logs"
        ),
        trades_log_file: Optional[str] = typer.Option(
            None, "--trades-log-file",
            help="Filename for trades logs"
        ),
        errors_log_file: Optional[str] = typer.Option(
            None, "--errors-log-file",
            help="Filename for errors logs"
        ),
        enable_async: Optional[bool] = typer.Option(
            None, "--enable-async/--disable-async",
            help="Enable asynchronous logging"
        ),
        max_file_size_mb: Optional[int] = typer.Option(
            None, "--max-file-size-mb",
            help="Maximum file size before rotation in MB"
        ),
        backup_count: Optional[int] = typer.Option(
            None, "--backup-count",
            help="Number of backup files to keep"
        ),
        enable_structured_logging: Optional[bool] = typer.Option(
            None, "--enable-structured-logging/--disable-structured-logging",
            help="Enable structured JSON logging"
        ),
        enable_contextual_logging: Optional[bool] = typer.Option(
            None, "--enable-contextual-logging/--disable-contextual-logging",
            help="Enable contextual field inclusion"
        ),
        mask_sensitive_data: Optional[bool] = typer.Option(
            None, "--mask-sensitive-data/--show-sensitive-data",
...
def test_logging(
    ...
    log_level: Optional[str] = typer.Option(None, "--log-level", "-l", 
        help="Logging level (DEBUG, INFO, WARNING, ERROR, CRITICAL)"),
    log_directory: Optional[str] = typer.Option(None, "--log-directory",
        help="Directory path for log files"),
    expert_log_file: Optional[str] = typer.Option(None, "--expert-log-file",
        help="Filename for expert logs"),
    trades_log_file: Optional[str] = typer.Option(None, "--trades-log-file",
        help="Filename for trades logs"),
    errors_log_file: Optional[str] = typer.Option(None, "--errors-log-file",
        help="Filename for errors logs"),
    enable_async: Optional[bool] = typer.Option(None, "--enable-async/--disable-async",
        help="Enable asynchronous logging"),
    max_file_size_mb: Optional[int] = typer.Option(None, "--max-file-size-mb",
        help="Maximum file size before rotation in MB"),
    backup_count: Optional[int] = typer.Option(None, "--backup-count",
        help="Number of backup files to keep"),
    enable_structured_logging: Optional[bool] = typer.Option(
        None, "--enable-structured-logging/--disable-structured-logging",
        help="Enable structured JSON logging"),
    ...
    # Log a test message
    logger.info("Logging system initialized via CLI", 

...
from .processors import add_context, add_log_level, add_timestamp, mask_sensitive_data, ensure_domain_context
...
class MetaLogger:
    ...
    def _setup_structlog(self):
        ...
        processors = [
            ...
            mask_sensitive_data if self.config.mask_sensitive_data else lambda x, y, z: z,  # Only add if masking is enabled
            structlog.processors.UnicodeDecoder(),
        ]

        # Add JSON formatter if structured logging is enabled
        if self.config.enable_structured_logging:
            processors.append(get_json_formatter())
        else:
            processors.append(get_console_formatter())
    ...
    def _setup_loggers(self):
        ...
        if self.config.enable_async:
            from .handlers.async_file import AsyncExpertFileHandler, AsyncTradesFileHandler, AsyncErrorsFileHandler
        else:
            from .handlers.file import ExpertFileHandler, TradesFileHandler, ErrorsFileHandler
        ...
        if hasattr(self.config, 'disk_space_check') and self.config.disk_space_check:
            expert_handler = DiskSpaceMonitoringFileHandler(
                os.path.join(self.config.log_directory, self.config.expert_log_file),
                console_handler=logging.StreamHandler(sys.stdout) if self.config.log_to_console else None,
                low_space_threshold_mb=100  # Configurable threshold
            )
            expert_handler.setLevel(self.config.log_level)
            expert_formatter = self._get_formatter_for_file()
            expert_handler.setFormatter(expert_formatter)
        else:
        ...
        if hasattr(self.config, 'disk_space_check') and self.config.disk_space_check:
            ...
            trades_formatter = self._get_formatter_for_file()
        ...
        if hasattr(self.config, 'disk_space_check') and self.config.disk_space_check:
            errors_handler = DiskSpaceMonitoringFileHandler(
                os.path.join(self.config.log_directory, self.config.errors_log_file),
                console_handler=logging.StreamHandler(sys.stderr) if self.config.log_to_console else None,
                low_space_threshold_mb=100  # Configurable threshold
            )
            ...
            errors_handler.setLevel(logging.ERROR)
        errors_formatter = self._get_formatter_for_file()
        ...
        if self.config.log_to_console:
            ...
            console_formatter = self._setup_console_formatter()
   ...
   def _setup_async(self):
       ...
       if self.config.enable_async:
           ...
           self.async_logger_thread = threading.Thread(target=self._async_log_worker, daemon=True)
